...
- What if the data in the answer doesn't contain the one data you need?
- What if the data does not support geo-questions, while you want to know which other objects are close to an object?
...
<aside class = "example">
The Cultural Heritage Agency of the Netherlands (RCE) has no direct link to the BAG, but each monument contains an address. However, the address details are difficult to compare:
...
The semantic layer describes how the data in the system relates to each other. This description contains both the meaning of the data and its coherence, as well as the knowledge about which data is in which API and how you approach that API.

The system catalog could theoretically be used for this, provided it is integrated with the actual definitions of semantic models such as that of the BAG, which is available as linked data.
...
2. APIs can also return identifiers from associated base registrations.
...
** Translated **: API, give me the NHR object with [BAG residence object id] as address.
...
Earlier we wrote that technical agreements have to be made for the laying of references or ** links ** at the data instance level, from one object to another, also between objects from different registrations.
...
In addition to the actual placement of the links, the semantic interpretation of this link is also an important subject that must be addressed. The current modeling of data within the * own silo * provides a certain freedom in modeling. However, when reference is made to data from other registrations, governance about the meaning of this reference is also necessary.

There is currently no uniform way to have external data refer to an object in basic registrations. If, for example, reference is made to a BAG property when publishing a dataset, there is no indication of how this link should be mentioned. As a result, it is now difficult with the external dataset to directly understand which dataset they are referring to.

A possible solution for this is a standardized name for the reference, for example, `` relatedBAGPand ''.
...
Considering this, current APIs are no more than 3 star data - the data is available on the web with an open license, is machine-readable, and is available in an open format. The only Dutch open government (geo) data that has four stars is Land Registry Linked Data via PDOK. Five star geodata is not there yet!

## Conclusions and recommendations

The starting point of the research was to build a demonstrator on top of APIs. But it turns out there are some snags.

### API versus knowledge graph

The majority of open geodata is made available as a map layer, which cannot be used as an API. There was only one API that scored well enough on the [maturity checklist] (# maturity) to be useful. The rest of the data used in the demonstrator was published in its own API during the project.
...
Data sets are usually not linked in current practice. For example, many registrations, instead of a BAG identifier, still contain fields where addresses are included as text. These addresses do not match 100% with the BAG.

In order to be able to query the object registrations in a coherent manner, it is a * basic condition * that the data sets at the level of individual objects are linked together using identifiers. Preferably these are included in the form of URIs.

* Recommendation *: Arrange governance for the one-time installation and subsequent management of these links - this costs time and money.
...
* Recommendation *: Take as a starting point in a system of related registrations ** 5 Star Open Data **. The absence of the 5th star automatically implies the lack of coherence.